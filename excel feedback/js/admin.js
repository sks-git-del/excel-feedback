...
async function loadAllAdminSubjects() {
  const gridElement = document.getElementById('all-subjects-grid');
  ...
  try {
    ...
    sortedDocs.forEach((doc) => {
      ...
      subject.id = doc.id;
      gridElement.appendChild(createAdminSubjectCard(subject));
    });
...
function createAdminSubjectCard(subject) {
  ...
  const formDate = subject.createdAt ? new Date(subject.createdAt).toLocaleDateString() : '-';
  const createdBy = subject.created_by === 'admin' ? '🔧 Admin' : `👨‍🏫 ${subject.faculty_name || 'Faculty'}`;

  card.innerHTML = `
    <h3>${subject.subject_name || 'Unnamed Subject'}</h3>
    <p class="subject-meta">Semester: ${subject.semester || '-'} | Created: ${formDate} | ${createdBy}</p>
    <p class="subject-meta" id="admin-summary-${subject.id}">Average Score: loading... | Responses: loading...</p>
    <div class="subject-actions">
      <button class="btn btn-primary btn-small" type="button" onclick="openAdminUrl('${subject.form_link || ''}', 'Google Form')">Open Form</button>
      <button class="btn btn-secondary btn-small" type="button" onclick="openAdminUrl('${subject.sheet_link || ''}', 'Google Sheet')">View Sheet</button>
      <button class="btn btn-warning btn-small" type="button" onclick="downloadAdminReport('${subject.subject_name || 'subject'}', '${subject.sheet_link || ''}')">Download</button>
      <button class="btn btn-secondary btn-small" type="button" onclick="refreshAdminSummary('${subject.id}', '${encodeURIComponent(subject.sheet_link || '')}')">Recalculate Avg</button>
      <button class="btn btn-danger btn-small" type="button" onclick="deleteAdminForm('${subject.id}')">Delete</button>
    </div>
  `;
...
async function loadAnalytics() {
  ...
  try {
    const [usersRes, subjectsSnapshot] = await Promise.all([
